...
import InputDniRepitente from "./InputDniRepitente"
import { initialValueNewStudentRepitente, validationSchemaNewStudentRepitente } from "./InitiaValidationRepitente"
import stateRepitente from "./StateRepitente"
const FormRepitente = () => {
  const { buttonDisableRepitente, onSubmitStudentRepitente } = stateRepitente()
  //La misma interfas, por que son los mismos datos
  const onSubmitSend = (data: NewStudent, helper: FormikHelpers<NewStudent>) => {
    const { resetForm } = helper
    onSubmitStudentRepitente(data, resetForm)
  }
      ...
      <div className="flex-[0_1_28rem] min-h-[40rem] rounded-3xl shadow-lg bg-white">
        <>
          <Formik
            ...
            initialValues={initialValueNewStudentRepitente}
            validationSchema={validationSchemaNewStudentRepitente}
          >
            {({ errors, touched, values, setFieldValue }) => (
              <Form className="font-titleFontLogin flex flex-col min-h-[40rem] items-center justify-center gap-3 p-4">
                ...
                <InputDniRepitente errors={errors} setFields={setFieldValue} touched={touched} dni={values.dni} />
                <Field disabled={true} name="nombres" className={`pl-2 border  focus:bg-blue-600/10 outline-none w-[20rem] text-black/50 h-[2.5rem] rounded-md ${errors.nombres && touched.nombres ? 'border-red-500/30' : ''
                  }`} placeholder="Nombres" />
                <Field disabled={true} name="apellidos" className={`pl-2 border focus:bg-blue-600/10 outline-none w-[20rem] text-black/50 h-[2.5rem] rounded-md ${errors.apellidos && touched.apellidos ? 'border-red-500/30' : ''
                ...
                <Field disabled={true} name="telefono" className={`text-black/50 pl-2 
              ...
              border outline-none w-[9.7rem] h-[2.5rem]
               focus:bg-blue-600/10 rounded-md text-black/50 ${errors.seccion && touched.seccion ? 'border-red-500/30' : ''
                ...
                <div className="flex gap-2 mt-4 flex-col items-end">
                  {/* <p className="text-center w-[13rem] cursor-pointer bg-blue-600/10 hover:bg-blue-600 hover:text-white transition rounded-lg text-black/40">Información del repitente</p> */}
                  <button disabled={buttonDisableRepitente} type="submit" className={`w-[20rem] rounded-md border bg-blue-400/80 ${buttonDisableRepitente ? "" : "hover:bg-blue-900"} flex items-center justify-center gap-2 h-[2.5rem] text-white hover:text-white transition`}>
